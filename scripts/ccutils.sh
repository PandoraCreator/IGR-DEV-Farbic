...
# Lifecycle event wait (five-server: peers need orderer:8001 + CCAAS:8003 reachable).
# Set FABRIC_WAIT_FOR_EVENT=false to submit without waiting, then queryapproved.
_lifecycle_wait_flags() {
  LIFECYCLE_WAIT_FLAGS=()
  if [[ "${FABRIC_WAIT_FOR_EVENT:-true}" == "false" ]]; then
    # Must be =false (single arg); bare "--waitForEvent false" still waits for deliver.
    LIFECYCLE_WAIT_FLAGS=(--waitForEvent=false)
  else
    LIFECYCLE_WAIT_FLAGS=(--waitForEvent=true --waitForEventTimeout="${FABRIC_EVENT_TIMEOUT:-300s}")
  fi
}
