...
set -x
peer channel fetch 0 $BLOCKFILE -o localhost:${FABRIC_ORDERER_PORT:-9001} --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA" >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
