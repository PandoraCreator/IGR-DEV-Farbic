...
function printHelp() {
  ...
  if [ "$USAGE" == "prereq" ]; then
    ...
    println "    -c <channel name> - Name of channel to create (defaults to \"igrchannel\")"
    ...
    println
    println " Examples:"
    println "   network.sh up createChannel -ca -c igrchannel -s couchdb "
    println "   network.sh up createChannel -bft -c igrchannel -s couchdb "
