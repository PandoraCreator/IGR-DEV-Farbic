#!/usr/bin/env bash
# After four remote peers are up: join channel + anchors (runs on peer-4 OPS).
...
# IGRBank peer1 (peer-5 when peer-4 is OPS-only)
if [[ "${PEER4_ROLE:-ops_only}" == "ops_only" ]]; then
  export CORE_PEER_ADDRESS=peer1.IGRBank.example.com:${PEER5_PORT_PEER}
  peer channel join -b "$BLOCK"
else
