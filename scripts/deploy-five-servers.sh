...
echo ">>> Phase 3: install Fabric binaries on peer-5 (download)"
eval "PW5=\${PEER5_PASSWORD}"
igr_ssh 5 "$(printf '%s\n' "
set -e
...
echo ">>> Phase 4: peer-5 Fabric CAs / enroll / orderer / OSN channel join"
PW5_ESC=$(printf '%q' "$PW5")
igr_ssh 5 "export SUDO_PASS=${PW5_ESC}; export CHANNEL_NAME=${CHANNEL_NAME:-mychannel}; export PATH=\$HOME/bin:\$HOME/fabric-samples/bin:\$PATH; bash ${IGR_NETWORK}/scripts/remote-peer5-bootstrap.sh"

echo ">>> Phase 5: copy MSP to peer-1..4"
TMPMSP=$(mktemp -d)
trap 'rm -rf "$TMPMSP"' EXIT
mkdir -p "$TMPMSP/organizations"
eval "H5=\$PEER5_HOST PW5=\$PEER5_PASSWORD"
sshpass -p "$PW5" ssh -p "${SSH_PORT}" -o StrictHostKeyChecking=accept-new "${SSH_USER}@${H5}" \
...
echo ">>> Phase 6: start CouchDB + peer on peer-1..4"
for n in 1 2 3 4; do
...
echo ">>> Phase 7: peer channel join + anchors (peer-5)"
eval "PW5=\${PEER5_PASSWORD}"
PW5_ESC=$(printf '%q' "$PW5")
igr_ssh 5 "export CHANNEL_NAME=${CHANNEL_NAME:-mychannel}; export PATH=\$HOME/bin:\$HOME/fabric-samples/bin:\$PATH; bash ${IGR_NETWORK}/scripts/remote-peer5-channel-join-anchors.sh"

echo ">>> Done. Verify on peer-5:"
