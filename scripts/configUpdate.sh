...
fetchChannelConfig() {
  ...
  set -x
  # Loopback on OPS host avoids hairpin/timeouts to orderer.example.com:8001 on the same VM.
  peer channel fetch config ${TEST_NETWORK_HOME}/channel-artifacts/config_block.pb -o ${FABRIC_ORDERER_HOST:-localhost}:${FABRIC_ORDERER_PORT} --ordererTLSHostnameOverride orderer.example.com -c $CHANNEL --tls --cafile "$ORDERER_CA"
  { set +x; } 2>/dev/null
...
createConfigUpdate() {
  ...
  if ! configtxlator compute_update --channel_id "${CHANNEL}" --original ${TEST_NETWORK_HOME}/channel-artifacts/original_config.pb --updated ${TEST_NETWORK_HOME}/channel-artifacts/modified_config.pb --output ${TEST_NETWORK_HOME}/channel-artifacts/config_update.pb; then
    { set +x; } 2>/dev/null
    fatalln "No configuration differences detected — anchor peer may already be set for this org"
  fi
