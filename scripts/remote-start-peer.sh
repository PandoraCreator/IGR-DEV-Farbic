#!/usr/bin/env bash
# Run on peer-1 … peer-4 with PEER_NODE=1|2|3|4 and SUDO_PASS set.
...
export PEER_NODE="${PEER_NODE:?need PEER_NODE 1-4}"
...
4)
  if [[ "${PEER4_ROLE:-}" == "ops_only" ]]; then
    ...
    echo "Do not start a peer here. Run: bash scripts/remote-peer5-bootstrap.sh" >&2
    exit 0
  fi
