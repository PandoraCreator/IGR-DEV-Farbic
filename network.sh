...
function createOrgs() {
  ...
  if [ "$CRYPTO" == "Certificate Authorities" ]; then
    ...
    infoln "Creating IGRPrimary Identities"

    createIGRPrimary

    infoln "Creating IGRBank Identities"
  ...
  infoln "Generating CCP files for IGRPrimary and IGRBank"
...
function networkUp() {
  ...
  if [ "${DATABASE}" == "couchdb" ]; then
    COMPOSE_FILES="${COMPOSE_FILES} -f compose/${COMPOSE_FILE_COUCH} -f compose/compose-couch-volumes.yaml"
  fi
